...
const KonselingAI = ({ onBack }) => {
  const [messages, setMessages] = useState([
    { 
      role: 'assistant', 
      content: 'Halo! Saya Konselor AI sekolahmu 🎓. Saya di sini untuk membantu masalah akademik, pemilihan jurusan, atau rencana karirmu ke depan. \n\nApa yang ingin kamu diskusikan hari ini?' 
    }
  ])
  ...
  const handleSend = async () => {
    ...
    try {
      const history = []
      // System prompt injection for Konselor AI
      history.push({
        role: 'user',
        parts: [{ text: "Kamu adalah seorang Konselor Sekolah (BK) yang profesional, empati, dan ahli dalam bimbingan karir serta akademik. Bantu siswa dengan saran yang bijak dan terstruktur." }],
      })
      history.push({
        role: 'model',
        parts: [{ text: "Siap, saya akan bertindak sebagai Konselor BK yang profesional dan empatik." }],
      })
      ...
      console.error("Konseling Error:", error)
    ...
    <div className="w-full max-w-7xl mx-auto h-screen bg-[#F8FAFC] flex flex-col overflow-hidden lg:px-10">
      <div className="px-6 pt-8 pb-4 flex items-center justify-between lg:mb-8 bg-[#F8FAFC]/80 backdrop-blur-xl lg:bg-transparent lg:backdrop-blur-none sticky top-0 z-10">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-2xl lg:text-4xl font-black text-brand-900 leading-tight">Konseling AI</h2>
            <div className="flex items-center gap-2 mt-1">
              <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span>
              <p className="text-xs lg:text-sm font-bold text-slate-400">Bimbingan Karir & Akademik</p>
            </div>
          </div>
        </div>
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="flex gap-4 items-center bg-white p-5 lg:p-7 rounded-[2rem] rounded-tl-none border border-slate-50 shadow-sm">
              ...
              <p className="text-sm lg:text-base font-bold text-slate-400 italic">Konselor sedang mengetik...</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-6 lg:p-10 bg-white/50 backdrop-blur-xl border-t border-slate-100">
        <div className="relative max-w-4xl mx-auto">
          <input 
            ...
            placeholder="Tanyakan tentang jurusan, karir, atau sekolah..."
        ...
        <p className="text-center mt-6 text-[10px] lg:text-xs font-bold text-slate-400 flex items-center justify-center gap-2">
          <Sparkles className="w-3 h-3" /> Konselor AI memberikan saran bimbingan secara objektif.
        </p>
      </div>
    </div>
  )
